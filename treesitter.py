...
class Treesitter(ABC):
    def __init__(self, language: LanguageEnum):
        ...
        if language == LanguageEnum.CSHARP:
            # Manual loading of the C♯ parser shared library.
            import os
            from tree_sitter import Language, Parser

            # Assuming the shared library is in the same directory as treesitter.py.
            csharp_lib_path = os.path.join(
                os.path.dirname(__file__), "libtree-sitter-c-sharp.dylib"
            )
            self.language_obj = Language(csharp_lib_path, "c_sharp")
            self.parser = Parser()
        ...
        # Attempt to set up the documentation query. If it fails, warn and skip.
        if "doc_query" in self.query_config:
            try:
                self.doc_query = self.language_obj.query(self.query_config["doc_query"])
            except Exception as e:
                print(
                    f"[WARNING] Failed to create doc_query for {language}: {e}. Skipping documentation comments."
                )
                self.doc_query = None
        else:
            self.doc_query = None

        # Initialize the queries using the language object.
...
def test_csharp_parsing():
    sample_code = b"class Foo { void Bar() { } }"
    treesitter_parser = Treesitter.create_treesitter(LanguageEnum.CSHARP)
    class_nodes, method_nodes = treesitter_parser.parse(sample_code)
    print(
        f"[DEBUG] C# Sample: Found {len(class_nodes)} class nodes and {len(method_nodes)} method nodes."
    )
