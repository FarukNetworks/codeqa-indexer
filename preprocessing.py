...
NODE_TYPES = {
    ...
    "csharp": {"class": "class_declaration", "method": "method_declaration"},
        ...
        for file in files:
            ...
            if file_ext in WHITELIST_FILES:
                if file not in BLACKLIST_FILES:
                    ...
                    if language:
                        print(f"Loading file: {file_path} as {language}")  # Debug print
    ...
    for language, files in files_by_language.items():
        ...
        for file_path in files:
            try:
                ...
                print(f"Unable to read file: {file_path}. Skipping. Error: {e}")
                continue
                ...
                class_data.append(
                    {
                        ...
                        "method_declarations": (
                            "\n-----\n".join(class_node.method_declarations)
                ...
                method_data.append(
                    {
                        "file_path": file_path,
                        "class_name": (
                            method_node.class_name if method_node.class_name else ""
                        ),
                        "name": method_name,
            ...
            while stack:
                ...
                if node.type == "identifier":
                    ...
                    if (
                        ...
                        and parent
                        and parent.type
                        in ["type", "class_type", "object_creation_expression"]
                    ):
                    ...
                    if (
                        ...
                        and parent
                        and parent.type in ["call_expression", "method_invocation"]
                    ):
...
def write_class_data_to_csv(class_data, output_directory):
    ...
    with open(output_file, "w", newline="", encoding="utf-8") as file:
        ...
        for row in class_data:
            ...
            row["references"] = "; ".join(
                [f"{ref['file']}:{ref['line']}:{ref['column']}" for ref in references]
            )
            writer.writerow(row)
